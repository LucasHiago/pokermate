				...
				<div class="panel-body">
					<div class="table-responsive" style="padding:0px;">
						...
						<?php foreach($aAgentList as $aAgent){ ?>
							<tr style="<?php echo $aCurrentAgent && $aCurrentAgent['id'] == $aAgent['id'] ? 'background:#f5f5f5;' : '' ?>"><td style="padding-top:0px;padding-bottom:0px;"><a href="<?php echo Url::to('home', 'agent/baoxian'); ?>?agentId=<?php echo $aAgent['id']; ?>&st=<?php echo $st; ?>" style="display:block;width:100%;height:45px;line-height:45px;"><?php echo $aAgent['agent_name']; ?></a></td><td style="width:62px;"><button class="btn btn-sm btn-danger" onclick="delAgent(this, <?php echo $aAgent['id']; ?>);">删除</button></td></tr>
						<?php } ?>
				...
				<div class="panel-body">
					...
					<div class="row h30">
						<div class="form-group" style="position:relative;top:-10px;">
							...
							<a class="btn btn-sm btn-primary" href="<?php echo Url::to('home', 'agent/export-baoxian'); ?>?agentId=<?php echo $aCurrentAgent['id']; ?>&st=<?php echo $st; ?>" style="float:right;">导出代理数据</a>
							<?php } ?>
						</div>
					</div>
					<div class="h10"></div>
...
<script type="text/javascript">	
	...
	function cleanAgentFencheng(o){
		...
		AlertWin.showAgentClean(<?php echo $aCurrentAgent ? $aCurrentAgent['id'] : 0 ?>, '<?php echo $aCurrentAgent ? $aCurrentAgent['agent_name'] : '' ?>', aId, selectTotalFencheng, <?php echo json_encode($aMoneyTypeList); ?>, '<?php echo $st; ?>', true);
		return;
		if(confirm('确定要清账？')){
			ajax({
	...
	function selectTimeEvent(){
		<?php if($aCurrentAgent){ ?>
			location.href = Tools.url('home', 'agent/baoxian') + '?agentId=<?php echo $aCurrentAgent['id']; ?>' + '&st=' + $('.J-agseltime-val').val();
		<?php }else{ ?>
			location.href = Tools.url('home', 'agent/baoxian') + '?st=' + $('.J-agseltime-val').val();
		<?php } ?>
	}
